...
const printLeet = async (req, res, quality, imdbcode, userslist) => {
    try {
        ...
        var urlID = `https://api.themoviedb.org/3/find/${imdbcode}?api_key=844dba0bfd8f3a4f3799f6130ef9e335&external_source=imdb_id`
        var imdbID = await axiosQuery(urlID, 'imdb_id')
        console.log(1)
        if ( !imdbID )
            return res.sendStatus(404)
        var restReq = await axiosQuery(`http://localhost:5000/api/movies/`);
...
const printYTS = async (baseURL, req, res, quality, userslist) => {
    try {
        ...
        var urlID = `https://api.themoviedb.org/3/find/${req.params.imdbcode}?api_key=844dba0bfd8f3a4f3799f6130ef9e335&external_source=imdb_id`
        var imdbID = await axiosQuery(urlID, 'imdb_id')
        if ( !imdbID )
            return res.sendStatus(404)
